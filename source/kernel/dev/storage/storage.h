...
public:
  ...
  /*
   * Since these functions may call Task::Wait() internally, threads calling these functions 
   * must be executed on TaskWithStack.
   */
  ...
  template<class T>
  IoReturnState Write(T &buf, size_t offset) __attribute__((warn_unused_result));
  virtual IoReturnState Read(uint8_t *buf, size_t offset, size_t size) __attribute__((warn_unused_result)) = 0;
  virtual IoReturnState Write(uint8_t *buf, size_t offset, size_t size) __attribute__((warn_unused_result)) = 0;
protected:
...
public:
  ...
  IoReturnState Register(Storage *dev) __attribute__((warn_unused_result));
  IoReturnState GetDevice(char *str, Storage *&dev);
private:
  StorageCtrl() {
  }
  ~StorageCtrl() {
