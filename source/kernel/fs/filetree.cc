 ...
 */
IoReturnState DiskFileTree::InodeCtrl::Alloc(InodeContainer::FileType type,
...
IoReturnState DiskFileTree::InodeCtrl::Get(InodeContainer &icontainer,
                                                uint32_t inum) {
  Locker locker(_lock);
  Inode *empty = nullptr;
 ...
 */
const char *DiskFileTree::GetNextPathNameFromPath(const char *path,
                                                       char *name) {
  while (*path == '/') {
    path++;
  }
 ...
 */
IoReturnState DiskFileTree::ReadDataFromInode(uint8_t *data,
                                                   InodeContainer &inode,
                                                   size_t offset,
                                                   size_t &size) {
  InodeContainer::Stat stat;
  RETURN_IF_IOSTATE_NOT_OK(inode.GetStatOfInode(stat));
  if (stat.type == InodeContainer::FileType::kDevice) {
    // TODO impl
...
#include <tty.h>
#include <libglobal.h>
IoReturnState DiskFileTree::LookupInodeFromPath(InodeContainer &inode,
                                                     const char *path,
                                                     bool parent) {
  ...
  while (1) {
    ...
    if (st.type != InodeContainer::FileType::kDirectory) {
