 ...
 private:
  ...
  class V6fsInode {
   public:
    ...
    V6fsInode() = delete;
    V6fsInode(int index, Storage &storage, SuperBlock &sb)
        : _index(index), _storage(storage), _sb(sb) {}
    void Init(Type type) {
      _st.type = type;
      _st.major = 0;
      _st.minor = 0;
    ...
    IoReturnState Read() __attribute__((warn_unused_result)) {
      // WE SHOULD VERIFY STRUCT HERE!!!
      return _storage.Read(
          _st, _sb.inodestart * kBlockSize + sizeof(V6fsInodeStruct) * _index);
    }
    IoReturnState Write() __attribute__((warn_unused_result)) {
      return _storage.Write(
          _st, _sb.inodestart * kBlockSize + sizeof(V6fsInodeStruct) * _index);
    }
    IoReturnState GetStatOfInode(InodeContainer::Stat &stat);
    const int _index;
    Storage &_storage;
    SuperBlock &_sb;
  };
  class V6fsInodeCtrl {
   public:
    ...
    V6fsInodeCtrl(Storage &storage, SuperBlock &sb)
        : _storage(storage), _sb(sb) {}
    IoReturnState Alloc(InodeNumber &inum, Type type)
        __attribute__((warn_unused_result));
    IoReturnState GetStatOfInode(InodeNumber inum,
                                 InodeContainer::Stat &stat)
        __attribute__((warn_unused_result)) {
  ...
  class BlockCtrl {
   public:
    ...
    IoReturnState ClearBlock(BlockIndex index)
        __attribute__((warn_unused_result));
  ...
  static Type ConvType(InodeContainer::FileType type) {
    switch (type) {
      case InodeContainer::FileType::kDirectory:
        return Type::kDirectory;
      case InodeContainer::FileType::kFile:
        return Type::kFile;
      case InodeContainer::FileType::kDevice:
        return Type::kDevice;
    };
    assert(false);
  }
  static InodeContainer::FileType ConvType(Type type) {
    switch (type) {
      case Type::kDirectory:
        return InodeContainer::FileType::kDirectory;
      case Type::kFile:
        return InodeContainer::FileType::kFile;
      case Type::kDevice:
        return InodeContainer::FileType::kDevice;
      case Type::kFree:
        assert(false);
    };
    assert(false);
  ...
  IoReturnState ReadDataFromInode(uint8_t *data, V6fsInode &inode,
  ...
  IoReturnState DirLookup(V6fsInode &dinode, char *name, int &offset,
                          InodeNumber &inode)
      __attribute__((warn_unused_result));
