...
void UdpCtrl::SetupServer() {
  ...
  for (size_t i = 0; i < devices->GetLen(); i++) {
    ...
    gtty->Cprintf("start udp server on %s\n", (*devices)[i]);
    dev->SetReceiveCallback(network_cpu, make_uptr(new Function<NetDev *>([](NetDev *eth) {
            ...
            if (rpacket->GetBuffer()[12] == 0x08 && rpacket->GetBuffer()[13] == 0x06) {
              ...
              if(rpacket->GetBuffer()[21] == 0x01 && (memcmp(rpacket->GetBuffer() + 38, my_addr, 4) == 0)) {
                // ARP Request
                uint8_t data[] = {
...
void UdpCtrl::Send(uint8_t (*target_addr)[4], uint16_t target_port, const char *data, size_t len) {
  ...
  // source port
  uint8_t source_port[] = {0x30, 0x39}; // 1234
  memcpy(buf + offset, source_port, 2);
  offset += 2;

  // target port
  // TODO analyze from argument
  uint8_t target_port_[] = {target_port >> 8, target_port & 0xFF};
...
static const int kClientPort = 12345;

void UdpCtrl::SetupKvsServer() {
  ...
  for (size_t i = 0; i < devices->GetLen(); i++) {
    ...
    gtty->Cprintf("start kvs server on %s\n", (*devices)[i]);
    dev->SetReceiveCallback(network_cpu, make_uptr(new Function2<NetDev *, UdpCtrl *>([](NetDev *eth, UdpCtrl *that) {
            NetDev::Packet *rpacket;
            if(!eth->ReceivePacket(rpacket)) {
              return;
            }
            ...
            if (rpacket->GetBuffer()[12] == 0x08 && rpacket->GetBuffer()[13] == 0x00) {
              ...
              do {
                ...
                // target address
                uint8_t taddress[4];
                taddress[0] = eth_data[16];
                taddress[1] = eth_data[17];
                taddress[2] = eth_data[18];
                taddress[3] = eth_data[19];
                ...
                if (protocol == 17) {
                  ...
                  if (tport == 12346) {
                    char *buf = const_cast<char *>(reinterpret_cast<const char *>(data->GetRawPtr()));
                    size_t len = udp_data_length;
                    if (strncmp("read ", buf + sizeof(uint64_t), 5) == 0) {
                      char *buf_tmp = buf;
                      buf_tmp += 5 + sizeof(uint64_t);

                      char *buf_end = buf_tmp;
                      while(*buf_end != '\0' && *buf_end != '\r' && *buf_end != '\n' && *buf_end != ' ') {
                        buf_end++;
                      }
                      size_t key_len = buf_end - buf_tmp;
                      char *data = (char *)masstree_get(tree, (const void *)buf_tmp, key_len);
                      if (data == nullptr) {
                        data = (char *)"NOT SET\n";
                      }
                      ...
                      char tbuf[len];
                      memcpy(tbuf, buf, sizeof(uint64_t));
                      memcpy(tbuf + sizeof(uint64_t), data, data_len + 1);

                      that->Send(&haddress, kClientPort, (const char *)tbuf, len);
                    } else if (strncmp("write ", buf + sizeof(uint64_t), 6) == 0) {
                      char *buf_tmp = buf;
                      buf_tmp += 6 + sizeof(uint64_t);
                      ...
                      char *buf_end = buf_tmp + index + 1;
                      while(*buf_end != '\0' && *buf_end != '\n' && *buf_end != ' ') {
                        buf_end++;
                      }

                      size_t value_len = buf_end - buf_tmp - index;
                      char *value_buf = new char[value_len];
                      memcpy(value_buf, buf_tmp + index + 1, value_len);
      
                      size_t key_len = index;
                      masstree_put(tree, (const void *)buf_tmp, key_len, value_buf);
